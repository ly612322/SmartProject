...
try:
    ...
    if form.getvalue("工序1") is not None:
        工序1 = str(form.getvalue("工序1"))
    ...
    if out_state == "":
        ...
        查询1 = " select isnull(LOT,'')LOT,不良模式,count(*)数量 FROM T_工程检 WHERE  UP_DATE BETWEEN CONVERT(DATETIME, '" + 开始日期 + "') AND CONVERT(DATETIME,'" + 结束日期 + "')"
        ...
        查询1 = 查询1 + " GROUP BY LOT,不良模式 ORDER BY LOT"
        ...
        查询 = " select isnull(LOT,'')LOT,不良模式,count(*)数量 FROM T_工程检 WHERE  1=1 AND LOT in  (" + LOT_list[2:] + "') AND 工段='" + 工段1 + "' GROUP BY LOT,不良模式 ORDER BY LOT"
