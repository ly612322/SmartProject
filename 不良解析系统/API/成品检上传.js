class Product {
    ...
    selectFile(event) {
        ...
        Array.from(this.files).forEach((element) => {
            let one = this.find(element.name, '-', 0);
            let two = this.find(element.name, '-', 1);
            let three = this.find(element.name, '-', 2);
            let fourth = this.find(element.name, '-', 3);
        ...
        this.fileName.forEach(element => {
            ...
            if (element.slice(second + 1, third) == 0) { // 无LOT号
                ...
                let paneltd = trtd.insertCell();
                paneltd.innerHTML = element.slice(third + 1, third + 3) //PANEL状态
                paneltd.setAttribute('data-c', element.slice(four, four + 2))
        ...
        Array.from(rows1).forEach((ele, index) => {
            ...
            let fenze = ele.childNodes[8];
            let fz_select = document.createElement('select');
            fz_select.style.width = '100%';
            fz_select.style.height = '100%';
            let _ = true;
            let flag = true;
            tdpm.onclick = () => {
                if (!_) return;
                ...
                if (kind.value == '') {
                    if (this.badindex != '') {
                        ...
                        ajax.onload = () => {
                            let response = JSON.parse(ajax.response);
                            let data = response.data;
                            let fenzelist = `<option value=''></option>`;
                            Array.from(data).forEach(element => {
                                if (element['不良模式'] == option[this.badindex].innerHTML) {
                                    fenzelist = fenzelist + `<option>${element['分责']}</option>`;
                                }
                            })
                            fz_select.innerHTML = fenzelist;
                            fenze.appendChild(fz_select);
                        }
                    }
                }
                flag = false
        ...
        for (let i = 1; i <= [...this.fileName].length; i++) {
            ...
            let tableNoLot = table.rows[i].cells[0].innerHTML + '-' //表格内无LOT号 取到0
                ...
                table.rows[i].cells[3].innerHTML + '-' + table.rows[i].cells[6].innerHTML;
            tableNoLot = tableNoLot + table.rows[i].cells[6].getAttribute('data-c');
            for (let j = 0; j < this.newfiles.length; j++) { //循环筛选过的图片
                ...
                let forth = this.find(this.newfiles[j].name, '-', 3);
                let td_简称 = this.newfiles[j].name.slice(0, first);

                if (this.newfiles[j].name.slice(second + 1, third) == 0) { //为无LOT号
                    let LOTNO = this.newfiles[j].name.slice(0, forth + 2); //     
